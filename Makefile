...
#endif
export	TOPDIR SRCTREE OBJTREE SPLTREE SB_SRCTOP SB_OBJTOP
...
SUBDIR_TOOLS = tools
# OCTEON removed: SUBDIR_EXAMPLES = examples/standalone examples/api
...
# OCTEON removed: SUBDIRS += $(SUBDIR_EXAMPLES)
endif
...
ifeq ($(CPU),octeon)
LIBS-y += arch/mips/cpu/octeon/cvmx/libcvmx.o
LIBS-y += board/octeon/common/libocteon.o
endif
ifeq ($(CPU),ixp)
...
LIBS-y += drivers/power/libpower.o \
	...
	drivers/power/pmbus/libpmbus.o \
...
#/* for JUNOS */
ALL-$(CONFIG_CRC_U_BOOT) += $(obj)u-boot-crc.bin  

ifeq ($(CPU),octeon)
# by Juniper change rename to $(obj)rename
ALL-y	+= archive $(obj)rename
ALL-$(CONFIG_CRC_U_BOOT) += $(obj)rename_crc  
endif
...
all:		$(ALL-y)
# OCTEON removed: $(SUBDIR_EXAMPLES)
...
$(obj)u-boot.bin:	$(obj)u-boot
		...
		$(BOARD_SIZE_CHECK)
ifeq ($(CPU),octeon)
#by juniper, we don't use this octeon_header
ifeq ($(VENDOR),octeon)
		$(OBJTREE)/tools/update_octeon_header $(OBJTREE)/u-boot.bin $(BOARD) \
			$(CONFIG_OCTEON_HEADER_ARGS) \
...
$(obj)u-boot-crc.bin :	$(obj)u-boot.bin
		$(obj)tools/mkimage -A $(ARCH) -T firmware -C none \
		-O u-boot -h -a $(CONFIG_SYS_TEXT_BASE) \
...
archive: $(ARCHIVE_FILES)
	-@if [ "${BOOTLOADER_ARCHIVE_DIRECTORY}" != "" ] ; then \
		echo "Archive directory: ${BOOTLOADER_ARCHIVE_DIRECTORY}/$(CPU)_$(BOARD)" ; \
		mkdir -p ${BOOTLOADER_ARCHIVE_DIRECTORY}/$(CPU)_$(BOARD) ; \
		cp -f $(ARCHIVE_FILES) ${BOOTLOADER_ARCHIVE_DIRECTORY}/$(CPU)_$(BOARD)/ ; \
		chmod -R a+r ${BOOTLOADER_ARCHIVE_DIRECTORY}/$(CPU)_$(BOARD) ; \
		touch ${BOOTLOADER_ARCHIVE_DIRECTORY}/$(CPU)_$(BOARD); \
	fi ;

		# Rename the u-boot files to match the configuration that was built
$(obj)rename: $(obj)u-boot $(obj)u-boot.bin $(obj)System.map
	mv $(obj)u-boot $(obj)u-boot-$(CPU)_$(BOARD)
	mv $(obj)u-boot.bin $(obj)u-boot-$(CPU)_$(BOARD).bin
	mv $(obj)u-boot.map $(obj)u-boot-$(CPU)_$(BOARD).map
	mv $(obj)System.map $(obj)System-$(CPU)_$(BOARD).map
	mv $(obj)u-boot.srec $(obj)u-boot-$(CPU)_$(BOARD).srec
	@echo "%%% ${BOARD} configuration built %%%"
	@${SIZE} -d $(obj)u-boot-$(CPU)_${BOARD}

$(obj)rename_crc: $(obj)u-boot-crc.bin
	mv $(obj)u-boot-crc.bin $(obj)u-boot-$(VENDOR)_$(BOARD)_crc.bin
...
#OCTEON removed: $(SUBDIR_EXAMPLES): $(obj)u-boot
...
$(VERSION_FILE):
		...
		@( localvers='$(shell $(TOPDIR)/tools/setlocalversion $(TOPDIR))' ; \
			...
			"$(U_BOOT_VERSION)" "$${localvers}" "$(CONFIG_VENDOR_U_BOOT_VER_APPEND)" ; \
		) > $@.tmp
...
clobber:	tidy
	...
	@rm -f $(obj)u-boot.pbl
	@rm -f $(obj)u-boot.imx
	@rm -f $(obj)u-boot-octeon*
	@rm -f $(obj)System-octeon_*.map
	...
	@rm -fr $(obj)include/asm/proc
ifneq ($(CPU),octeon)
	@rm -fr $(obj)include/asm/arch $(obj)include/asm
endif
