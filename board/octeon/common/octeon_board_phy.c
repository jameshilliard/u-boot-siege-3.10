...
static int octeon_fdt_marvell_config(const struct eth_device *ethdev)
{
	...
	if (!fdt_node_check_compatible(gd->fdt_blob, phy_node_offset,
		...
		/* The Linux kernel will power down some clause 22 PHYs.  If
		 * powered down, power them back up.
		 */
		...
		if (value & BMCR_PDOWN) {
			value &= ~BMCR_PDOWN;
			value |= BMCR_RESET;
			phy_write(oct_eth_info->phydev, MDIO_DEVAD_NONE, 0,
				  value);
			mdelay(10);
		}
	}
...
const char const * broadcom_devices[] = {
	...
	"broadcom,bcm8727",
	"broadcom,bcm84856",
	"broadcom,bcm54616s", /* added for airbus */
	NULL
};
...
static int octeon_fdt_generic_8023_c22_config(const struct eth_device *ethdev)
{
	...
	/* Check for compatible generic 802.3 clause 22 PHY */
	if (fdt_node_check_compatible(gd->fdt_blob, phy_node_offset,
				      "ethernet-phy-ieee802.3-c22"))
		return 0;
	 ...
	 * powered down, power them back up.
	 */
	value = phy_read(oct_eth_info->phydev, MDIO_DEVAD_NONE, MII_BMCR);
	if (value & BMCR_PDOWN) {
		value &= ~BMCR_PDOWN;
		value |= BMCR_RESET;
		phy_write(oct_eth_info->phydev, MDIO_DEVAD_NONE, 0, value);
		mdelay(10);
	}
