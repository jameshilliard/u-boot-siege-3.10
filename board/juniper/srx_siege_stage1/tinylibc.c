...
{
    uart_write_byte(digits[data/16]);
    uart_write_byte(digits[data&0xf]);
}

void print_hex_uint32(uint32_t data)
{
    uint32_t tmp = data;
...
int printf(const char *fmt __attribute__((unused)), ...)
{
    return 0;
}
#else /* Juniper srx_siege don't use these funcs */
...
#undef read_c0_count
#endif
#ifdef read_c0_compare
#undef read_c0_compare
#endif
#ifdef write_c0_compare
#undef write_c0_compare
#endif
