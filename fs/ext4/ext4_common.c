...
unsigned long blk_dread(struct block_dev_desc *block_dev, lbaint_t start,
			lbaint_t blkcnt, void *buffer)
{
	return block_dev->block_read(block_dev->dev, start, blkcnt, buffer);
}

unsigned long blk_dwrite(struct block_dev_desc *block_dev, lbaint_t start,
			 lbaint_t blkcnt, const void *buffer)
{
	return block_dev->block_write(block_dev->dev, start, blkcnt, buffer);
}
...
static inline void ext4fs_bg_set_free_inodes(struct ext2_block_group *bg,
					const struct ext_filesystem *fs,
					uint32_t count)
{
	bg->free_inodes = cpu_to_le16(count & 0xffff);
	if (fs->gdsize == 64)
		bg->free_inodes_high = cpu_to_le16(count >> 16);
}

static inline void ext4fs_bg_free_inodes_dec
	(struct ext2_block_group *bg, const struct ext_filesystem *fs)
{
	uint32_t free_inodes = ext4fs_bg_get_free_inodes(bg, fs);
	free_inodes--;
	ext4fs_bg_set_free_inodes(bg, fs, free_inodes);
}

static inline uint32_t ext4fs_bg_free_blocks_get(
					const struct ext2_block_group *bg,
					const struct ext_filesystem *fs)
{
...
static inline void ext4fs_bg_free_blocks_set(
					struct ext2_block_group *bg,
					const struct ext_filesystem *fs,
					uint32_t count)
{
	bg->free_blocks = cpu_to_le16(count & 0xffff);
	if (fs->gdsize == 64)
		bg->free_blocks_high = cpu_to_le16(count >> 16);
}
...
static inline void ext4fs_bg_free_blocks_dec
	(struct ext2_block_group *bg, const struct ext_filesystem *fs)
{
	uint32_t free_blocks = ext4fs_bg_free_blocks_get(bg, fs);
	free_blocks--;
	ext4fs_bg_free_blocks_set(bg, fs, free_blocks);
}

static inline uint32_t
ext4fs_get_bg_itable_unused(const struct ext2_block_group *bg,
			 const struct ext_filesystem *fs)
{
...
static inline void ext4fs_set_bg_itable_unused(struct ext2_block_group *bg,
					  const struct ext_filesystem *fs,
					  uint32_t count)
{
	bg->bg_itable_unused = cpu_to_le16(count & 0xffff);
	if (fs->gdsize == 64)
		bg->bg_itable_unused_high = cpu_to_le16(count >> 16);
}

static inline void ext4fs_bg_itable_unused_dec
	(struct ext2_block_group *bg, const struct ext_filesystem *fs)
{
	uint32_t free_inodes = ext4fs_get_bg_itable_unused(bg, fs);
	free_inodes--;
	ext4fs_set_bg_itable_unused(bg, fs, free_inodes);
}
...
restart:
		...
		bg_flags = ext4fs_bg_get_flags(bgd);
		b_bitmap_blk = ext4fs_bg_get_block_id(bgd, fs);
		if (bg_flags & EXT4_BG_BLOCK_UNINIT) {
...
int ext4fs_get_new_inode_no(void)
{
	...
	if (!journal_buffer || !zero_buffer)
		goto fail;
	has_gdt_chksum = le32_to_cpu(fs->sb->feature_ro_compat) &
	...
	if (fs->first_pass_ibmap == 0) {
		for (i = 0; i < fs->no_blkgrp; i++) {
			...
			if (free_inodes) {
				...
				if (has_gdt_chksum)
					ext4fs_set_bg_itable_unused(bgd, fs,
								free_inodes);
...
restart:
		...
		bg_flags = ext4fs_bg_get_flags(bgd);
		i_bitmap_blk = ext4fs_bg_get_inode_id(bgd, fs);
		...
		if (has_gdt_chksum) {
			...
			if (fs->gdsize == 64)
				bgd->bg_itable_unused_high =
						bgd->free_inodes_high;
		}
		ext4fs_sb_free_inodes_dec(fs->sb);
		goto success;
	}
