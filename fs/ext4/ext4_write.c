...
int ext4_write_file_compat(const char *filename, void *buf, int offset, int len)
{
	loff_t len_written = 0;
	int ret;

	ret = ext4_write_file(filename, buf, (int)offset, (int)len,
			      &len_written);
	return ret ? -1 : (int)len_written;
}
