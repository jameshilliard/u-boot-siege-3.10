...
int ext4fs_probe(struct block_dev_desc *fs_dev_desc,
...
int ext4_read_file_compat(const char *filename, void *buf, int offset, int len)
{
	loff_t len_read = 0;
	int ret;

	ret = ext4_read_file(filename, buf, (loff_t)offset, (loff_t)len,
			     &len_read);

	return ret ? -1 : (int)len_read;
}
