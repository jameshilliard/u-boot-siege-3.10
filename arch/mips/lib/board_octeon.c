...
#if defined(_ABIO32)
static void relocate_code_octeon_o32_arg(ulong stack_virt,
                              ulong gd_virt,
                              uint64_t xkphy_addr,
                              int map_size);
#endif
...
int uart_debug(const char *fmt, ...)
{
	...
	/* Print the string */
	uart_write_string(printbuffer);
	uart_write_string("\r\n");
	return i;
}

#ifdef OCTEON_ASSEMBLE_DEBUG 
int o32_warning(char *func, int32_t line)
{
    /* calling N32 designed assemble functions in o32 ABI compile*/
    uart_debug("#### warning %s, %d possible assemble error\r\n", func, line);
    return 0;
}
#endif
...
#else
		gd->cpu_clk = juniper_dmult(ref_clock , npei_dbg_data.s.c_mul);
        debug("%s: line %d ref_clock: %llu, c_mul: %llu\n", __func__, __LINE__, ref_clock , npei_dbg_data.s.c_mul);
#endif
...
#else
		gd->cpu_clk = juniper_dmult(ref_clock , rst_boot.s.c_mul);
        debug("%s: line %d ref_clock: %llu, c_mul: %llu\n", __func__, __LINE__, ref_clock , rst_boot.s.c_mul);
		gd->bus_clk = juniper_dmult(ref_clock , rst_boot.s.pnr_mul);
#endif
	} else if (octeon_has_feature(OCTEON_FEATURE_PCIE)) {
		cvmx_mio_rst_boot_t mio_rst_boot;
		mio_rst_boot.u64 = cvmx_read_csr(CVMX_MIO_RST_BOOT);
#if !defined(CONFIG_SRX_SIEGE) || defined(_ABIO32)
		gd->cpu_clk = ref_clock * (uint32_t)mio_rst_boot.s.c_mul;
		gd->bus_clk = ref_clock * (uint32_t)mio_rst_boot.s.pnr_mul;
#else
		gd->cpu_clk = juniper_dmult(ref_clock , mio_rst_boot.s.c_mul);
        debug("%s: line %d ref_clock: %llu, c_mul: %llu\n", __func__, __LINE__, ref_clock , mio_rst_boot.s.c_mul);
		gd->bus_clk = juniper_dmult(ref_clock , mio_rst_boot.s.pnr_mul);
#endif
	} else {
...
#else
		gd->cpu_clk = juniper_dmult(ref_clock , dbg_data.s.c_mul);
        debug("%s: line %d ref_clock: %llu, c_mul: %llu\n", __func__, __LINE__, ref_clock , dbg_data.s.c_mul);
#endif
		gd->bus_clk = gd->cpu_clk;
	}
	debug("%s: cpu: %llu, bus: %llu\n", __func__, gd->cpu_clk, gd->bus_clk);
...
static void board_init_f_finish(ulong bootflag)
{
	...
	debug("Reserving %dk for STACK_SIZE \n",
	      (STACK_SIZE)>> 10);
...
#ifdef CONFIG_SRX_SIEGE
    bd->bi_i2c_id = (uint32_t)gd->arch.board_desc.board_type;  /* board i2c_id */
#endif
...
#else
    u32_map_size = (map_size & 0xffffffff);
	relocate_code_octeon_o32_arg(stack_virt, (void *)(u32)gd_virt,
			     MAKE_XKPHYS(addr), u32_map_size);
#endif
}
...
/* 
 * for O32 compiling;
 * relocate_code_octeon will pass 64bit arg to N32 assemble codes;
 * when assemble codes is compiled by O32, will make mistake;
 * To make O32 assemble codes work, make this function, only pass 
 * 2 32bit args to assemble codes
 */
static void
relocate_code_octeon_o32_arg( ulong stack_virt,
                              ulong gd_virt,
                              uint64_t xkphy_addr,
                              int map_size)
{
    reloc_arg_t reloc_arg;
    reloc_arg_t *arg_addr;
    ulong addr_high, addr_low;

    reloc_arg.stack_virt = stack_virt;
    reloc_arg.gd_virt = gd_virt;
    reloc_arg.xkphy_addr_high = (xkphy_addr >>32);
    reloc_arg.xkphy_addr_low  = (xkphy_addr & 0xffffffff);
    reloc_arg.map_size = map_size;

    arg_addr = &reloc_arg;
    addr_high = (((uint64_t)arg_addr) >> 32);
    addr_low  = (((uint64_t)arg_addr) & 0xffffffff);

	relocate_code_octeon_o32(addr_high, addr_low);
}
#endif
...
#ifndef _ABIO32
void board_init_r(gd_t * gd_virt, u64 dest_addr)
#else
void board_init_r(gd_t * gd_virt, ulong dest_addr)
#endif
{
...
# endif
	...
	debug("Copy FDT to final location \r\n");
...
# define OCTEON_ENV_FLASH_SIZE (0x10000UL)
    ...
    /* init the bootbus local memory for debug exception and also save ebase to local memory */
...
#if defined(CONFIG_API) && defined(CONFIG_SRX_SIEGE)
       /* Initialize API */
       api_init();
#endif
