...
#else
void relocate_code_octeon(ulong, ulong, uint64_t, ulong)
    __attribute__((noreturn));

typedef struct reloc_arg_s{
    uint32_t stack_virt;
    uint32_t gd_virt;
    uint32_t xkphy_addr_high;
    uint32_t xkphy_addr_low;
    uint32_t map_size;
} reloc_arg_t;

/* pass the addr_high addr_low of the arg structure to assemble codes */
void relocate_code_octeon_o32(ulong arg_addr_high, ulong arg_addr_low)
    __attribute__((noreturn));
