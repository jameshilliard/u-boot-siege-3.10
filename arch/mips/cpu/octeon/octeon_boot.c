				   ...
				   uint64_t byte_count_arg)
{
    ...
    volatile uint64_t val64 = ((uint64_t) val_high << 32) | val_low;
#endif

#ifndef _ABIO32
    /* default octeon codes using N32 ABI to compile */
	asm volatile ("	.set	push			\n"
		      "	.set	mips64			\n"
		      "	.set	noreorder		\n"
		      /* Now do real work..... */
		      ...
		      : "memory");
#else
    /* for o32 compile,use 2 32bit register to generate the 64bit value */
	volatile uint32_t count_low = byte_count & 0xffffffff;
	volatile uint32_t count_high = byte_count >> 32;

	volatile uint32_t start_low = start_addr & 0xffffffff;
	volatile uint32_t start_high = start_addr >> 32;
	...
	asm volatile ("	.set	push			\n"
		      ...
		      "	dsll	%[startl], 32		\n"
		      "	dsrl	%[startl], 32		\n"
		      "	daddu	%[starth], %[starth], %[startl] \n"
		      /* Combined value is in starth */
		      ...
		      [startl] "r"(start_low), [starth] "r"(start_high),
		      [vall] "r"(val_low), [valh] "r"(val_high)
		      : "memory");
...
void octeon_bzero64_pfs(uint64_t start_addr_arg, uint64_t count_arg)
{
	...
	if (!OCTEON_IS_OCTEON1PLUS()) {
        ...
        /* default octeon codes is using N32 ABI */
		asm volatile ("	.set	push			\n"
			      "	.set	arch=octeon2		\n"
			      "	.set	noreorder		\n"
			      /* Now do real work..... */
			      ...
			      : : "memory");
#else
        /* for juniper o32 compiler, we don't support zcbt inst. */
        WATCHDOG_RESET();
        while (count >= 0x400) {
            memset64(start_addr, 0, 0x400);
            count -= 0x400;
            start_addr += 0x400;
            WATCHDOG_RESET();
        }
...
#endif
    /* 
     * Juniper siege products need loader use network and usb
     * we can't disable CONFIG_OCTEON_INTERNAL_ENET, CONFIG_OCTEON_MGMT_ENET
     * and CONFIG_USB_OCTEON simplly, we need these modules in uboot;
     */
#ifndef CONFIG_SRX_SIEGE 
#ifdef CONFIG_OCTEON_INTERNAL_ENET
