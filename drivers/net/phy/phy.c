...
int board_phy_init(void) __attribute__((weak));
...
int
get_phy_id(struct mii_dev *bus, int addr, int devad, uint32_t *phy_id)
...
int phy_reset(struct phy_device *phydev)
	__attribute__((weak, alias("__phy_reset")));

int __phy_reset(struct phy_device *phydev)
{
	int reg;
	int timeout = 500;
	...
	if (phydev->drv->reset)
		return phydev->drv->reset(phydev);
