...
#ifdef CONFIG_OCTEON
	cacheline_size = max(cacheline_size, size);
#endif
...
int xhci_alloc_virt_device(struct usb_device *udev)
{
	...
	debug("%s: dcbaa at %p slot 0x%x: context at %p (0x%llx)\n", __func__,
	      ctrl->dcbaa,
	      slot_id, virt_dev->out_ctx->bytes, byte_64);
...
#endif
	debug("Setting DCBAA to %p (0x%llx)\n", ctrl->dcbaa, val_64);
...
#ifdef CONFIG_OCTEON
	val_64 = virt_to_phys(ctrl->event_ring->dequeue) & (u64)~ERST_PTR_MASK;
#else
	val_64 = (u64)deq & (u64)~ERST_PTR_MASK;
#endif
#ifdef CONFIG_SRX_SIEGE
	debug("Writing ring dequeue pointer 0x%lx (0x%llx)\n", deq, val_64);
#else
	debug("Writing ring dequeue pointer 0x%x (0x%llx)\n", deq, val_64);
#endif
	/* Update HC event ring dequeue pointer */
	xhci_writeq(&ctrl->ir_set->erst_dequeue, val_64);
...
#ifdef CONFIG_OCTEON
	val_64 |= virt_to_phys(ctrl->erst.entries) & (u64)~ERST_PTR_MASK;
#else
	val_64 |= ((u32)(ctrl->erst.entries) & ~ERST_PTR_MASK);
#endif
	debug("Writing event ring pointer %p to 0x%x (0x%llx)\n",
	      &ctrl->ir_set->erst_base,
	      ((u32)ctrl->erst.entries) & ~ERST_PTR_MASK,
	      val_64);
				    ...
				    unsigned int ep_index)
{
	...
	debug("%s(%p, %p, %u) ctx type: %d, bytes: 0x%x\n", __func__, ctrl, ctx,
	      ep_index, ctx->type, ctx->bytes);
...
#ifdef DEBUG
	debug("%s: ctx: %p\n", __func__, ep_ctx);
	print_buffer(ep_ctx, ep_ctx, 4, sizeof(*ep_ctx) / 4, 0);
#endif
	return ep_ctx;
}
			...
			unsigned int ep_index)
{
	...
	debug("%s(%p, %p, %p, %d)\n", __func__, ctrl, in_ctx, out_ctx, ep_index);
