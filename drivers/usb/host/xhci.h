 ...
 */
/* based on http://git.denx.de/?p=u-boot.git;a=blob;f=drivers/usb/host/xhci.h;h=ceb1573d86b043c8d3c507a8eed55ca484bb14de;hb=refs/heads/next */
#ifndef HOST_XHCI_H_
#define HOST_XHCI_H_
...
#else
/* The Cavium OCTEON processor needs wrappers to access the 64-bit I/O space */
...
#if 1
	u64 val_lo = xhci_readl(ptr);
	u64 val_hi = xhci_readl(ptr + 1);
#else
	u64 val_lo = readl(ptr);
	u64 val_hi = readl(ptr + 1);
#endif
	debug("%s(%p) => 0x%llx\n", __func__, regs, val_lo + (val_hi << 32));
...
static inline void xhci_writeq(__le64 volatile *regs, const u64 val)
{
	...
	debug("%s(%p, 0x%llx)\n", __func__, regs, val);
#if 1
	xhci_writel(ptr, val_lo);
	xhci_writel(ptr  +1, val_hi);
#else
	writel(val_lo, ptr);
	writel(val_hi, ptr + 1);
#endif
