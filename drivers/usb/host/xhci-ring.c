 ...
 */
/* based on http://git.denx.de/?p=u-boot.git;a=blob;f=drivers/usb/host/xhci-ring.c;h=19c3ec62118d95c77957679de5cca93368ab24a3;hb=refs/heads/next */
#include <common.h>
						...
						bool more_trbs_coming)
{
	...
	debug("%s(%p, %p, %d)\n", __func__, ctrl, ring, more_trbs_coming);
...
{
	debug("%s(%p, %p) dequeue: %p\n", __func__, ctrl, ring, ring->dequeue);
	do {
		/*
							...
							u32 ep_state)
{
	...
	debug("%s(%p, %p, %d)\n", __func__, ctrl, ep_ring, ep_state);
...
/**
 * Function for queueing an address device command TRB on the command ring.
 * Check to make sure there's room on the command ring for one command TRB.
 *
 * @param ctrl		Host controller data structure
 * @param ptr		Pointer address to write in the first two fields (opt.)
 ...
 * @return none
 */
void xhci_queue_address_device(struct xhci_ctrl *ctrl, u8 *ptr, u32 slot_id)
{
	u32 fields[4];
	u64 val_64;
#ifdef CONFIG_OCTEON
	val_64 = virt_to_phys(ptr);
#else
	val_64 = (uintptr_t)ptr;
#endif
	debug("%s(%p, %p, %u)\n", __func__, ctrl, ptr, slot_id);
	BUG_ON(prepare_ring(ctrl, ctrl->cmd_ring, EP_STATE_RUNNING));

	fields[0] = lower_32_bits(val_64);
	fields[1] = upper_32_bits(val_64);
	fields[2] = 0;
	fields[3] = TRB_TYPE(TRB_ADDR_DEV) | SLOT_ID_FOR_TRB(slot_id) |
						ctrl->cmd_ring->cycle_state;
	debug("  fields: 0x%x 0x%x, 0x%x, 0x%x\n", fields[0], fields[1],
	      fields[2], fields[3]);
	queue_trb(ctrl, ctrl->cmd_ring, false, fields);
 ...
 * @return none
 */
void xhci_queue_configure_endpoint(struct xhci_ctrl *ctrl, u8 *ptr, u32 slot_id,
				   trb_type cmd)
{
	u32 fields[4];
	u64 val_64;
...
union xhci_trb *xhci_wait_for_event(struct xhci_ctrl *ctrl, trb_type expected)
{
	...
	do {
		...
		/* TODO: remove this once enumeration has been reworked */
			...
			printf("Unexpected XHCI event TRB, type: %d, expected: %d, skipping... "
				"(%08x %08x %08x %08x)\n",
			        type, expected,
...
static void abort_td(struct usb_device *udev, int ep_index)
{
	...
	debug("%s(%p (%d), %d)\n", __func__, udev, udev->slot_id, ep_index);
	...
	if ((TRB_TO_SLOT_ID(field) != udev->slot_id) ||
		...
		printf("Error: Mismatch slot ID or index, %d != %d, field: 0x%x, index: 0x%x, expect 0x%x\n",
		       TRB_TO_SLOT_ID(field), udev->slot_id, field,
		       TRB_TO_EP_INDEX(field), ep_index);
	if (GET_COMP_CODE(le32_to_cpu(event->trans_event.transfer_len)) != COMP_STOP)
		printf("Warning: transfer comp code 0x%x != 0x%x (COMP_STOP)\n",
			...
			int length, void *buffer)
{
...
#endif
	debug("%s(%p (%d), 0x%lx, %d, %p)\n", __func__, udev, udev->slot_id,
	      pipe, length, buffer);
	...
	if ((le32_to_cpu(ep_ctx->ep_info) & EP_STATE_MASK) == EP_STATE_HALTED) {
		printf("%s: ring halted\n", __func__);
		printf("  ep info: 0x%x, ep info2: 0x%x, deq: 0x%llx, tx_info: 0x%x\n",
		       le32_to_cpu(ep_ctx->ep_info),
		       le32_to_cpu(ep_ctx->ep_info2),
		       le64_to_cpu(ep_ctx->deq),
		       le32_to_cpu(ep_ctx->tx_info));
		printf("  deq_seg: %p, trb: %p\n", ring->deq_seg, ring->dequeue);
		printf("  ring: %p, buffer: 0x%llx, len: 0x%x, flags: 0x%x\n",
		       ring,
		       le64_to_cpu(ring->dequeue->trans_event.buffer),
		       le32_to_cpu(ring->dequeue->trans_event.transfer_len),
		       le32_to_cpu(ring->dequeue->trans_event.flags));
	}
