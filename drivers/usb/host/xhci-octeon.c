 ...
 */
/* by Juniper, change it from 3 to 100 */
# define CONFIG_OCTEON_DWC_USB_PLL_RETRY_COUNT	100
#endif
...
static int dwc3_uphy_pll_powercycle(int node, int index, int max_retries);
...
static void dwc3_core_soft_reset(struct dwc3 *dwc3_reg, int intf)
{
	...
	dwc3_writel(dwc3_reg, DWC3_GUSB2PHYCFG(0), reg);
        ...
        /* delay accordint to OCTEON spec */
	mdelay(10);
	...
	dwc3_writel(dwc3_reg, DWC3_GCTL, reg);
        ...
        /* delay according to OCTEON spec */
	mdelay(10);
}


int octeon3_xhci_phy_reset(struct xhci_hcor *hcor)
{
	uint64_t addr64;
	uint64_t node;
	union cvmx_usbdrdx_uctl_ctl uctl_ctl;
	int intf;
	ulong addr32 = (ulong)hcor;

	intf = (addr32 >> 16) & 1;
	node = (addr32 >> 17) & 7;

	addr64 = CVMX_USBDRDX_UCTL_CTL(intf);
	uctl_ctl.u64 = cvmx_read_csr_node(node, addr64);
	uctl_ctl.s.uphy_rst = 1;
	cvmx_write_csr_node(node, addr64, uctl_ctl.u64);
	cvmx_read_csr_node(node, addr64);
	udelay(20);
...
int xhci_hcd_init(int index, struct xhci_hccr **hccr, struct xhci_hcor **hcor)
{
	...
	do {
#ifdef CONFIG_SRX_SIEGE
        /* to support both siege and airbus USB config at 1 dts tree */
        if (OCTEON_IS_MODEL(OCTEON_CN73XX)) {
