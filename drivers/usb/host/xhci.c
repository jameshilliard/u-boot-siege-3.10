 ...
 * GPL sources:
 * based on http://lxr.free-electrons.com/source/drivers/usb/host/xhci.c?v=3.10
 */
...
int octeon_xhci_csr_timeout = 0;

extern int octeon3_xhci_phy_reset(struct xhci_hcor *hcor);


/* Check for csrtimeout in uahc_gsts register */
static int xhci_check_csrtimeout(struct usb_device *udev)
{
	...
	base = &hcor->or_usbcmd;
	base -= 0x20 / sizeof(*gsts);
	gsts = base + 0xc118 / sizeof(*gsts);
	guid = base + 0xc128 / sizeof(*guid);

	val = xhci_readl(gsts);
	if (val & 0x20) {
		/* Set the global failure flag */
		octeon_xhci_csr_timeout = 1;
	
		xhci_writel(gsts, val);
		xhci_writel(guid, 1);
		val = xhci_readl(gsts);
					...
					uint32_t done, int usec)
{
	...
	debug("%s(%p, 0x%x, 0x%x, %d)\n", __func__, ptr, mask, done, usec);
	do {
...
void xhci_powerdown_fixup(volatile uint32_t *status_reg) __attribute__((weak));
 ...
 */
void __xhci_powerup_fixup(volatile uint32_t *status_reg)
{
	mdelay(50);
}

void xhci_powerup_fixup(volatile uint32_t *status_reg)
	__attribute__((weak, alias("__xhci_powerup_fixup")));
...
static int xhci_configure_endpoints(struct usb_device *udev, bool ctx_change)
{
	...
	debug("%s(%p (%d), %d)\n", __func__, udev, udev->slot_id, ctx_change);
...
static int xhci_set_configuration(struct usb_device *udev)
{
	...
	for (cur_ep = 0; cur_ep < num_of_ep; cur_ep++) {
		...
		ep_ctx[ep_index]->ep_info2 |=
			...
			(le16_to_cpu((get_unaligned(&endpt_desc->wMaxPacketSize)))));
...
static int xhci_address_device(struct usb_device *udev)
{
	...
	xhci_queue_address_device(ctrl, (void *)ctrl_ctx, slot_id);
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
		...
		u16 wIndex, volatile uint32_t *addr, u32 port_status)
{
	...
	debug("%s(0x%x, 0x%x, %p, 0x%x)\n", __func__, wValue, wIndex, addr,
	      port_status);
...
#if defined(CONFIG_OCTEON_USB_OCTEON3)
extern int octeon_xhci_test_usb2(struct xhci_hcor *hcor, int dev, int index);
static int xhci_test_usb2(struct xhci_hcor *hcor, int dev, int index)
{
	uint32_t cmd;
	int ret;
	...
	return octeon_xhci_test_usb2(hcor, dev, index);
}
#endif
			...
			void *buffer, struct devrequest *req)
{
	...
	case USB_REQ_SET_FEATURE | ((USB_DIR_OUT | USB_RT_PORT) << 8):
		...
		case USB_PORT_FEAT_POWER:
			reg |= PORT_POWER;
			xhci_writel(status_reg, reg);
			xhci_powerup_fixup((uint32_t *)status_reg);
			break;
		...
		case USB_PORT_FEAT_TEST:
			if (xhci_test_usb2(hcor, udev->devnum - 1,
					   le16_to_cpu(req->index)) < 0)
				goto unknown;
			break;
#endif
		default:
	...
	case USB_REQ_CLEAR_FEATURE | ((USB_DIR_OUT | USB_RT_PORT) << 8):
		...
		case USB_PORT_FEAT_POWER:
			reg &= ~PORT_POWER;
			xhci_writel(status_reg, reg);
			xhci_powerdown_fixup(status_reg);
#ifdef CONFIG_SRX_SIEGE
			if (xhci_check_csrtimeout(udev)) {
				/* Identified failure. Try to recover */
				uint32_t oldreg = reg;
				debug("Resetting phy...");
				octeon3_xhci_phy_reset(hcor);
				debug(" Done.\n");
				debug("Writing 0x%x\n", oldreg | PORT_RESET);
				xhci_writel(status_reg, oldreg | PORT_RESET | PORT_POWER);
				mdelay(10);
				xhci_check_csrtimeout(udev);
				mdelay(10);
				debug("Read back 0x%x, writing 0x%x\n",
				      xhci_readl(status_reg), oldreg);
				xhci_writel(status_reg, oldreg);
				mdelay(10);
				reg = xhci_readl(status_reg);

				if (reg & PORT_POWER) {
					debug("Second write failed too!\n");
				} else {
					debug("%s: Second write succeeded, reg: 0x%x\n",
					      __func__, reg);
				...
				debug("Read back (success): 0x%x\n",
				      xhci_readl(status_reg));
			}
			debug("Read back (end): 0x%x\n",
			      xhci_readl(status_reg));
#endif
			break;
		case USB_PORT_FEAT_C_RESET:
...
submit_bulk_msg(struct usb_device *udev, unsigned long pipe, void *buffer,
								int length)
{
	debug("%s(%p, 0x%lx, %p, %d)\n", __func__, udev, pipe, buffer, length);
					...
					int length, struct devrequest *setup)
{
	...
	debug("%s(%p, 0x%lx, %p, %d, %p (request: %d))\n", __func__, udev,
	      pipe, buffer, length, setup, setup->request);
...
int usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
	...
	if (HCS_INDICATOR(reg))
		put_unaligned(get_unaligned(&descriptor.hub.wHubCharacteristics)
				| cpu_to_le16(0x80),
			      &descriptor.hub.wHubCharacteristics);
