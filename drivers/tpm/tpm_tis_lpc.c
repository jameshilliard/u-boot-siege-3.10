 ...
 * $Id$
 *
 * tpm_tis_lpc.c - Low Level Routines for TPM DEVICE
 *
 ...
 * All rights reserved.
 *
 * CREDITS: Google Chromium OS Authors
 * as committed by following first patch
 * http://git.denx.de/?p=u-boot.git;a=commit;
 * h=f626799816d04502cd69cc45d2add68de8b199bd
 *
...
u8 tpm_read_byte(const u8 *ptr)
	__attribute__((weak, alias("__tpm_read_byte")));

u8 __tpm_read_byte(const u8 *ptr)
{
	u8  ret = readb(ptr);
...
u32 tpm_read_word(const u32 *ptr)
	__attribute__((weak, alias("__tpm_read_word")));

u32 __tpm_read_word(const u32 *ptr)
{
	u32  ret = readl(ptr);
...
void tpm_write_byte(u8 value, u8 *ptr)
	__attribute__((weak, alias("__tpm_write_byte")));

void __tpm_write_byte(u8 value, u8 *ptr)
{
...
void tpm_write_word(u32 value, u32 *ptr)
	__attribute__((weak, alias("__tpm_write_word")));

void __tpm_write_word(u32 value, u32 *ptr)
{
 ...
 */
static u32 tis_wait_reg(u32 *reg, u8 mask, u8 expected, u8 byte_read)
{
	u32 time_us = MAX_DELAY_US; 
	volatile u32 word_value=0;
	volatile u8 byte_value=0;

	while (time_us > 0) {
		if (byte_read)
		{
		    byte_value = tpm_read_byte((u8 *) reg);

		    if ((byte_value & mask) == expected)
		    {
			return byte_value;
		    }
		} else {
		    word_value = tpm_read_word(reg);
		    if ((word_value & mask) == expected)
		    {
			return word_value;
		    }
		}
...
#ifdef TPM_DEBUG
	printf("\n Did not get expected value %x in addr %x", expected, reg);
#endif
	return TPM_TIMEOUT_ERR;
}

void print_dev_vendor_name(void)
{
	...
	vid = vendor_dev_id & 0xffff;
	did = (vendor_dev_id >> 16) & 0xffff;
...
#ifdef TPM_DEBUG
	printf("\n Vendor & Device ID - %04x", tpm_read_word(&lpc_tpm_dev[0].did_vid));
#endif
...
static u32 tis_senddata(const u8 * const data, u32 len)
{
	...
	value = tis_wait_reg(&lpc_tpm_dev[locality].tpm_status,
			     TIS_STS_COMMAND_READY, TIS_STS_COMMAND_READY, 0);
...
int tis_open(void)
{
	...
	tpm_write_byte(TIS_ACCESS_REQUEST_USE, (u8 *) &lpc_tpm_dev[locality].access);

	/* did we get a lock? */
	if (tis_wait_reg(&lpc_tpm_dev[locality].access,
			 TIS_ACCESS_ACTIVE_LOCALITY,
			 TIS_ACCESS_ACTIVE_LOCALITY, 1) == TPM_TIMEOUT_ERR) {
...
#ifdef TPM_DEBUG
	printf("\n COMMAND READY state written to Status Register \n");
#endif	
	return 0;
}
...
int tis_close(void)
{
	...
	if ((tpm_read_byte((u8 *) &lpc_tpm_dev[locality].access)) &
	    TIS_ACCESS_ACTIVE_LOCALITY) {
		tpm_write_byte(TIS_ACCESS_ACTIVE_LOCALITY,
			       (u8 *) &lpc_tpm_dev[locality].access);
