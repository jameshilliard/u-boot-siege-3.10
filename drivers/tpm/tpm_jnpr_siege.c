 ...
 * $Id$
 *
 * tpm.c -- High Level Routines for TPM device 
 *
...
uint32_t tpm_self_test_full(void)
{
	...
	return_val = tpm_sendrecv_command(command, NULL, NULL);

#ifdef TPM_DEBUG
	printf("\n tpm_self_test_full() returned %x \n\n", return_val);
#endif
...
#ifdef TPM_DEBUG
	printf("\n tpm_continue_self_test() returned %x", return_val);
#endif
...
uint32_t tpm_raw_transfer(uint8_t *command, uint8_t *response, size_t response_length)
{
	if (response == NULL) {
		return TPM_BAD_PARAMETER;
	}
	return tpm_sendrecv_command(command, response, &response_length);
}
