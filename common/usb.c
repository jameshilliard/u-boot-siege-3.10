...
static bool usb_controller_initialized[CONFIG_USB_MAX_CONTROLLER_COUNT] = {
	false,
};
...
retry:
	...
	/* init low_level USB */
#ifndef CONFIG_SWAP_USB_CTRL
	for (i = 0; i < CONFIG_USB_MAX_CONTROLLER_COUNT; i++) {
#else
	for (i = CONFIG_USB_MAX_CONTROLLER_COUNT-1; i >= 0; i--) {
#endif
		/* init low_level USB */
		printf("USB%d:   ", i);
...
int usb_stop(void)
{
	...
	if (usb_started) {
		...
		for (i = 0; i < CONFIG_USB_MAX_CONTROLLER_COUNT; i++) {
			if (!usb_controller_initialized[i])
				continue;
			if (usb_lowlevel_stop(i)) {
				printf("failed to stop USB controller %d\n", i);
			} else {
				usb_controller_initialized[i] = false;
				memset(&usb_dev[i], 0, sizeof(struct usb_device));
				usb_dev[i].devnum = -1;
			}
		}
...
usb_set_maxpacket_ep(struct usb_device *dev, int if_idx, int ep_idx)
{
	...
	ep_wMaxPacketSize = le16_to_cpu(get_unaligned(&ep->wMaxPacketSize));
			...
			unsigned char *buffer, int cfgno)
{
	...
	while (index + 1 < le16_to_cpu(dev->config.desc.wTotalLength) &&
	       head->bLength) {
		debug("%s: index: %d, total length: %d, head length: %d\n",
		      __func__, index,
		...
		case USB_DT_ENDPOINT:
			...
			ep_wMaxPacketSize = le16_to_cpu(get_unaligned(&dev->config.\
								if_desc[ifno].\
								ep_desc[epno].\
								wMaxPacketSize));
			     ...
			     unsigned char *buffer, int cfgno)
{
	...
	if (result < length)
		config->wTotalLength = cpu_to_le16(result);
