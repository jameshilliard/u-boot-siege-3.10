 ...
 * GPL sources:
 *
 * Based on http://git.denx.de/?p=u-boot.git;a=blob;f=common/cmd_ide.c;h=e185c95b26d6ce7bfc6bab5ea526ba8d6a55a2d6;hb=b05dcb58fe04c6274fc942fa93efe77072395951
 */
...
block_dev_desc_t ide_dev_desc[CONFIG_SYS_IDE_MAXBUS][CONFIG_SYS_IDE_MAXDEVICE];
/* ------------------------------------------------------------------------- */
...
uchar ide_wait(int dev, ulong t);
...
int do_ide (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	case 2:
		if (strncmp(argv[1],"res",3) == 0) {
			...
			for (busno = 0; busno < CONFIG_SYS_IDE_MAXBUS; busno++) {
				for (devno = 0;
				     devno < CONFIG_SYS_IDE_MAXDEVICE;
				     ++devno) {
					if (ide_dev_desc[busno][devno].type
					    == DEV_TYPE_UNKNOWN)
						continue; /* list only known devices */
					printf("IDE device %d: ",
					       IDE_DEVICE(busno, devno));
					dev_print(&ide_dev_desc[busno][devno]);
				}
			}
			return 0;
			...
			dev_print(&ide_dev_desc[IDE_BUS(curr_device)]
					       [IDE_DEV(curr_device)]);
			return 0;
			...
			for (busno = 0; busno < CONFIG_SYS_IDE_MAXBUS; ++busno) {
				for (devno = 0;
				     devno < CONFIG_SYS_IDE_MAXDEVICE;
				     ++devno) {
					if (ide_dev_desc[busno][devno].part_type
					    != PART_TYPE_UNKNOWN) {
						++ok;
						if (devno)
							putc ('\n');
						print_part(&ide_dev_desc[busno][devno]);
					}
				}
			}
			if (!ok) {
	...
	case 3:
		if (strncmp(argv[1], "dev", 3) == 0) {
			...
			if (ide_dev_desc[busno][devno].type == DEV_TYPE_UNKNOWN) {
				return 1;
			}
			...
			if (ide_dev_desc[busno][devno].part_type!=PART_TYPE_UNKNOWN) {
					print_part(&ide_dev_desc[busno][devno]);
			} else {
	...
	default:
		...
		if (strcmp(argv[1], "read") == 0) {
			...
			printf("\nIDE read: device %d block # %ld, count %ld ... ",
			       curr_device, blk, cnt);
#endif
			busno = IDE_BUS(curr_device);
			devno = IDE_DEV(curr_device);
			n = ide_dev_desc[busno][devno].block_read(curr_device,
								  blk, cnt,
								  (ulong *)addr);
			/* flush cache after read */
			flush_cache(addr,
				    cnt * ide_dev_desc[busno][devno].blksz);
...
int do_diskboot (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	show_boot_progress (45);
	if (get_partition_info (&ide_dev_desc[busno][devno], part, &info)) {
	...
	if (ide_dev_desc[busno][devno].block_read(dev, info.start,
						  1, (ulong *)addr) != 1) {
	...
	if (ide_dev_desc[busno][devno].block_read(dev, info.start + 1, cnt,
						  (ulong *)(addr+info.blksz))
	    != cnt) {
		printf("** Read error on %d:%d\n", dev, part);
...
void ide_init (void)
{
	...
	for (bus = 0; bus < CONFIG_SYS_IDE_MAXBUS; ++bus) {
		...
		for (dev = 0; dev < CONFIG_SYS_IDE_MAXDEVICE; dev++) {
			printf ("Bus %d: ", bus);

			/* Select device
			 */
			mdelay (100);		/* 100 ms */
			ide_outb(IDE_DEVICE(bus, dev),
				 ATA_DEV_HD, ATA_LBA | ATA_DEVICE(dev));
			mdelay (100);		/* 100 ms */
			i = 0;
			do {
				mdelay (10);		/* 10 ms */

				c = ide_inb(IDE_DEVICE(bus, dev), ATA_STATUS);
				i++;
				...
				if ((i >= 100) && ((i % 100) == 0))
					putc ('.');
#ifdef CONFIG_OCTEON
				/* On some Octeon boards if the CF is not
				 * installed then this register will be read
				 * as 0xff.
				*/
				if (c == 0xff)
					break;
	...
	for (bus = 0; bus < CONFIG_SYS_IDE_MAXBUS; bus++) {
		for (dev = 0; dev < CONFIG_SYS_IDE_MAXDEVICE; ++dev) {
#ifdef CONFIG_IDE_LED
			int led = (bus == 0) ? LED_IDE1 : LED_IDE2;
#endif
#ifdef CONFIG_OCTEON
			octeon_block_dev_desc_t *oct_dev =
				(octeon_block_dev_desc_t *)ide_dev_desc[bus][dev].priv;

			if (!ide_bus_ok[bus])
				continue;

			if (ide_dev_desc[bus][dev].if_type == IF_TYPE_UNKNOWN) {
				/* Only initialize if not initialized during
				 * controller scan
				 */
				ide_dev_desc[bus][dev].type = DEV_TYPE_UNKNOWN;
				ide_dev_desc[bus][dev].if_type = IF_TYPE_IDE;
				ide_dev_desc[bus][dev].dev = dev;
				ide_dev_desc[bus][dev].part_type = PART_TYPE_UNKNOWN;
				ide_dev_desc[bus][dev].blksz = 0;
				ide_dev_desc[bus][dev].lba = 0 ;
				ide_dev_desc[bus][dev].block_read = ide_read;
			}
			/* We don't support multiple CF cards, and the 16 bit
			 * boot bus read/modify/write hack breaks the
			 * primary/secondary selection, so ignore all
			 * non-primary CF cards
			 */
			if (((oct_dev->quirks & BLOCK_QUIRK_IDE_CF) &&
			     oct_dev->dev_on_bus == 1)
			    || dev != 0)
				continue;
#else
			...
			ide_ident(&ide_dev_desc[bus][dev]);
			ide_led (led, 0);		/* LED off	*/
			dev_print(&ide_dev_desc[bus][dev]);
/*			ide_print (i); */
			if ((ide_dev_desc[bus][dev].lba > 0) &&
			    (ide_dev_desc[bus][dev].blksz > 0)) {
				/* initialize partition type */
				debug("Initializing partition for dev %d:%d\n",
				      bus, dev);
				mdelay(200);
				init_part (&ide_dev_desc[bus][dev]);
				if (curr_device < 0)
					curr_device = IDE_DEVICE(bus, dev);
			}
		}
		WATCHDOG_RESET();
	}
...
block_dev_desc_t * ide_get_dev(int dev)
{
	...
	busno = IDE_BUS(dev);
	devno = IDE_DEV(dev);
	return (dev < CONFIG_SYS_IDE_MAXDEVICE * CONFIG_SYS_IDE_MAXBUS) ?
		&ide_dev_desc[busno][devno] : NULL;
}
...
# define ide_input_swap_data(x,y,z) ide_input_data(x,y,z)
#else
...
static void ide_ident (block_dev_desc_t *dev_desc)
{
	...
	ide_input_swap_data(device, iobuf, ATA_SECTORWORDS);

#ifdef CONFIG_OCTEON

	octeon_cf_config_dma_timing(device, iop);
#endif
...
#ifdef CONFIG_OCTEON
	{
		octeon_block_dev_desc_t *oct_dev = (octeon_block_dev_desc_t *)
			ide_dev_desc[IDE_BUS(device)][IDE_DEV(device)].priv;

		/* If DMA enabled, use dedicated read function */
		if (oct_dev->dma_type == CF_DMA_TYPE_MDMA)
			return octeon_ide_read_dma(device, blknr, blkcnt,
						   buffer);
	}
#endif
		 ...
		 const void *buffer)
{
...
#ifdef CONFIG_OCTEON
	{
		...
		debug("%s(%d, %llu, %lu, 0x%p)\n",
		      __func__, device, blknr, blkcnt, buffer);
		if (oct_dev->dma_type == CF_DMA_TYPE_MDMA)
			return octeon_ide_write_dma(device, blknr, blkcnt, buffer);
		debug("%s: Not using DMA mode for device %d, dma type: %d\n",
		      __func__, device, oct_dev->dma_type);
	}
#endif
	/* Select device
	 */
...
static void ide_reset(void)
{
	...
	for (busno = 0; busno < CONFIG_SYS_IDE_MAXBUS; ++busno) {
		...
		for (devno = 0; devno < CONFIG_SYS_IDE_MAXDEVICE; ++devno)
			ide_dev_desc[busno][devno].type = DEV_TYPE_UNKNOWN;
	}
...
int ide_device_present(int dev)
{
	...
	if (dev >= CONFIG_SYS_IDE_MAXBUS * CONFIG_SYS_IDE_MAXDEVICE)
		return 0;

	busno = IDE_BUS(dev);
	devno = IDE_DEV(dev);
	return (ide_dev_desc[busno][devno].type == DEV_TYPE_UNKNOWN ? 0 : 1);
}
#endif
/* ------------------------------------------------------------------------- */
...
unsigned char atapi_issue(int device,unsigned char* ccb,int ccblen, unsigned char * buffer,int buflen)
{
	...
	ide_output_data_shorts(device, (unsigned short *)ccb,ccblen/2); /* write command block */
	...
	if(n != 0) { /* data transfer */
		...
		if ((ide_inb(device, ATA_SECT_CNT) & 0x02) == 0) {
			...
			ide_output_data_shorts(device,(unsigned short *)buffer,n);
		} else {
			debug("Read from device @ %p shorts %d\n",buffer,n);
			ide_input_data_shorts(device,(unsigned short *)buffer,n);
		}
	}
