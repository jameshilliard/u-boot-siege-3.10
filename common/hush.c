...
#endif
extern char uboot_prompt[CONFIG_SYS_MAX_PROMPT_LEN];
char uboot_prompt_hush_ps2[CONFIG_SYS_MAX_PROMPT_LEN];
...
#  endif
	...
	if (i->promptmode == 1) {
		...
		if (prompt) {
			snprintf(uboot_prompt, prompt,
				 sizeof(uboot_prompt));
		} else if (uboot_prompt[0] == '\0') {
#ifdef CONFIG_SYS_PROMPT
			strncpy(uboot_prompt, CONFIG_SYS_PROMPT,
				sizeof(uboot_prompt));
#elif defined(CONFIG_SYS_PROMPT_HUSH_PS2)
			strncpy(uboot_prompt, CONFIG_SYS_PROMPT_HUSH_PS2,
				sizeof(uboot_prompt));
#else
# error Either CONFIG_SYS_PROMPT or CONFIG_SYS_PROMPT_HUSH_PS2 must be set
#endif
		}
		n = readline(uboot_prompt);
	} else {
		const char *prompt_ps2 = getenv("prompt_ps2");

		if (prompt_ps2) {
			snprintf(uboot_prompt_hush_ps2, prompt_ps2,
				 sizeof(uboot_prompt_hush_ps2));
		} else if (uboot_prompt[0] == '\0') {
#ifdef CONFIG_SYS_PROMPT
			strncpy(uboot_prompt_hush_ps2, CONFIG_SYS_PROMPT,
				sizeof(uboot_prompt_hush_ps2));
#elif defined(CONFIG_SYS_PROMPT_HUSH_PS2)
			strncpy(uboot_prompt_hush_ps2,
...
# error Either CONFIG_SYS_PROMPT or CONFIG_SYS_PROMPT_HUSH_PS2 must be set
		...
		n = readline(uboot_prompt_hush_ps2);
	}
#ifdef CONFIG_BOOT_RETRY_TIME
	if (n == -2) {
...
static int handle_dollar(o_string *dest, struct p_context *ctx, struct in_str *input)
{
	...
	if (isalpha(ch)) {
		...
		case '#':
			...
			break;
#endif
#if defined(__U_BOOT__) && defined(CONFIG_SYS_HUSH_PARSER_CAVIUM_EXTRA)
		case '(':
			/* Support $(var) syntax like ${var} */
			b_addchr(dest, SPECIAL_VAR_SYMBOL);
			ctx->child->sp++;
			b_getch(input);
